  ...
  useEffect(() => {
    if (showBrowseRecipes) {
      ...
      axios
        .get(`${API_ENDPOINTS.RECIPES}`, {
          params: {
            category: browseRecipesCategory === 'All' ? undefined : browseRecipesCategory,
            search: browseRecipesSearch || undefined,
          },
        })
        .then((res) => {
          // Filter out user-created recipes (they have 'name' not 'title')
          setBrowseRecipes(res.data.filter((r: any) => r.title || (!r.name && !r.title)));
...
function AddFoodModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.post(`${API_ENDPOINTS.FOODS_SEARCH.replace('/search', '')}`, payload);
    ...
    for (const line of lines) {
      try {
        ...
        if (res.data && res.data.length > 0) {
          matches.push({ ...res.data[0], original: line, quantity: 1, unit: 'g' });
        } else {
